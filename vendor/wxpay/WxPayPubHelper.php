...
{	
	function __construct($options)
	{
		...
		WxPayConf_pub::$APPID = isset($options['appid']) ? $options['appid'] : '';
		WxPayConf_pub::$MCHID = isset($options['mch_id']) ? $options['mch_id'] : '';
		WxPayConf_pub::$KEY = isset($options['key']) ? $options['key'] : '';
		WxPayConf_pub::$APPSECRET = isset($options['appsecret']) ? $options['appsecret'] : '';
		//设置curl超时时间
	...
	function getPrepayId()
	{
		$this->postXml();
		$this->result = $this->xmlToArray($this->response);
		$arr = ['code'=>1,'msg'=>'没有返回值','data'=>''];
		if($this->result){
			if($this->result['return_code'] == 'SUCCESS'){
				if($this->result['result_code'] == 'SUCCESS'){
					$prepay_id = $this->result["prepay_id"];
					$arr = ['code'=>0,'msg'=>'成功','data'=>$prepay_id];
				}else{
//					return $this->result['err_code'].'-'.$this->result['err_code_des'];
					$arr = ['code'=>1,'msg'=>$this->result['err_code'].'-'.$this->result['err_code_des'],'data'=>''];
				}
			}else{
				$arr = ['code'=>1,'msg'=>$this->result['return_msg'],'data'=>''];
