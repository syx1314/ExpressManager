...
class HuaweiPush
{
    ...
    public function __construct($appId, $appSecret)
    {
        ...
        $accesstoken = HdataCache::get();
        if (!$accesstoken) {
            $response = $this->RefreshToken();
            HdataCache::set($response['access_token']);
    ...
    protected function callRestful($url, $params)
    {
        ...
        $extra_conf = array(
            ...
            CURLOPT_CAINFO => dirname(__FILE__) . '/root.pem', // CA根证书getcwd()
     ...
     * 发送Push消息
     */
    public function SendPushMessage($deviceTokenList = array(), $payload = array(), $expireTime = '')
    {
        $urlParams = array();
        $nsp_ctx = array();
        $nsp_ctx['ver'] = '1';
...
class HdataCache
{
     ...
     * @access public
     */
    static public $cachetime = 604800;
    ...
    {
        self::setfile();
        if (is_file(self::$file) && (time() - filemtime(self::$file)) <= self::$cachetime) {
     ...
     */
    static public function set($content)
    {
        self::setfile();
    ...
    static public function setfile()
    {
        self::$file = dirname(__FILE__) . "/access_token.txt";
    }
}